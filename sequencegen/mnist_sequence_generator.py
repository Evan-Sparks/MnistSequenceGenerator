...
def generate_mnist_sequence(digits, spacing_range, image_width):
    ...
    """
    return _normalize_to_numpy(_generate_sequence_helper(digits, spacing_range, image_width))


def _generate_sequence_helper(digits, spacing_range, image_width):
    """
    ...
    if not reduce((lambda x, y: x and y), map((lambda z: isinstance(z, int) and 0 <= z < 10), sequence)):
    ...
    chosen_images = []
    for digit in digits:
        chosen_image_index = random.randrange(len(images[digit]))
        chosen_images.append(images[digit][chosen_image_index])
    return _concatenate_lists(_add_separators(chosen_images, images.num_rows, spacing_range, pad_left_config),
                              images.num_rows, image_width)
...
def _concatenate_lists(images, num_rows, num_cols):
    ...
    """
    output_array = [[0 for _ in range(num_cols)] for _ in range(num_rows)]
    ...
    while current_output_col < num_cols and current_image_index < len(images):
        ...
        for current_image_row in range(num_rows):
        ...
        if current_image_col == array_cols:
            ...
            while current_image_index < len(images) and len(images[current_image_index][0]) == 0:
                current_image_index += 1
    while current_output_col < num_cols:
        for current_image_row in range(num_rows):
            output_array[current_image_row][current_output_col] = 255
        current_output_col += 1
        current_image_col += 1
    return output_array


def _add_separators(images, num_rows, spacing_range, pad_left):
    """
    Insert random-width spacer lists between each lists in arrays.
    :param images: A list of digit image lists to place spacers between.
    :param num_rows: The number of rows in the image.
    ...
    for index in range(len(images)):
        if index != 0 or pad_left:
            spacing = random.randrange(spacing_range[0], spacing_range[1] + 1)
            output.append([[0 for _ in range(spacing)] for _ in range(num_rows)])
